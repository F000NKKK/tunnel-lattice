...
`0.1.0` опубликована (см. `index.md`, `SUPPORT.md`). Ничего в этом
...
блокируется на тех же асинхронных методах (`futures::executor::block_on`),
а не хранит второй хендл.
...
на устройство, перекладывающий `PacketIo::recv` в `futures::Stream`. Поэтому
новому backend'у не нужна собственная асинхронность, чтобы её получить:
одной реализации `PacketIo` уже достаточно для работы через
`tunnel-lattice-async`, а `AsyncPacketIo` — чисто аддитивная оптимизация для
backend'а с настоящим нативным путём; граница `Handle<D>`/`ConnectedDevice`
у фасада одинакова в обоих случаях (см. rustdoc `tunnel-lattice`).
...
них, вообще не тянет асинхронный рантайм. Известное ограничение: адаптер на
потоках не может принудительно отменить рабочий поток, заблокированный
внутри `recv` при отсутствии новых пакетов —
